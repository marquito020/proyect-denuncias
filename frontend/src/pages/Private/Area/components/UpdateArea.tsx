...
import { useGetArea, useUpdateArea } from "../../../../hooks/useArea";
...
function UpdateArea() {
  ...
  const { updatedArea, isMutating } = useUpdateArea(`${id}`);
  const { typesOfComplaints, isLoading, error } = useAllTypesOfComplainst();
  const { areaFound, isLoading: isLoadingArea } = useGetArea(`${id}`);
  ...
  useEffect(() => {
    if (typesOfComplaints) {
      if (areaFound) {
        const selectedDefault = areaFound.typesComplaintId.map(
          (typeComplaintId) => {
            const typeComplaint = typesOfComplaints.find(
              (element) => (element._id == typeComplaintId)
            );

            return { label: `${typeComplaint?.name}`, value: typeComplaintId };
          }
        );

        setSelectedTypesOfComplaint(selectedDefault);
      }
    }
  }, [areaFound, typesOfComplaints]);
  ...
  const handleSubmit = async (e: FormEvent<AreaCustomForm>) => {
    ...
    try {
      const response = await updatedArea({
        name,
        description,
        typesComplaintId,
      });
      ...
      if (response?._id) {
        ...
        toast.success("Área actualizado exitosamente!");
      } else {
      ...
      <form onSubmit={handleSubmit} className="card-body">
        ...
        <div className="form-control ">
          ...
          <Select
            isMulti
            required
            value={selectedTypesOfComplaint}
            options={options}
        ...
        <div className="form-control mb-4">
          ...
          <textarea
            ...
            defaultValue={areaFound?.description}
          ></textarea>
        </div>
